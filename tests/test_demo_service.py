...
def test_high_latency_keeps_shallow_health_green(demo_client):
    ...
    assert health.status_code == 200
    assert health.json()["status"] == "healthy"
    assert health.json()["mode"] == "high_latency"
    assert booking.status_code == 200
    assert booking.json()["service_mode"] == "high_latency"
    assert elapsed_ms >= 20


def test_dependency_failure_affects_health_and_booking(demo_client):
    configured = demo_client.post(
        "/admin/failure-mode",
...
def test_reset_restores_healthy_mode(demo_client):
    demo_client.post(
        "/admin/failure-mode",
        json={"mode": "dependency_failure", "latency_ms": 1},
    )
